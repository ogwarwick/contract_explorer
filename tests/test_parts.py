...
class TestPart10:
    """Integration tests for Part 10"""

    def test_part10_exists(self, lcha_structure):
        assert "10" in lcha_structure["parts"]

    def test_part10_has_sections(self, lcha_structure):
        sections = lcha_structure["parts"]["10"]["sections"]
        assert len(sections) > 0

    def test_part10_sections_have_required_fields(self, lcha_structure):
        sections = lcha_structure["parts"]["10"]["sections"]
        required = ["id", "parent_id", "type", "text_for_embedding", "references"]
        for section_id, section_data in sections.items():
            for field in required:
...
class TestPart11:
    """Integration tests for Part 11"""

    def test_part11_exists(self, lcha_structure):
        assert "11" in lcha_structure["parts"]

    def test_part11_has_sections(self, lcha_structure):
        sections = lcha_structure["parts"]["11"]["sections"]
        assert len(sections) > 0

    def test_part11_sections_have_required_fields(self, lcha_structure):
        sections = lcha_structure["parts"]["11"]["sections"]
        required = ["id", "parent_id", "type", "text_for_embedding", "references"]
        for section_id, section_data in sections.items():
            for field in required:
...
class TestPart12:
    """Integration tests for Part 12"""

    def test_part12_exists(self, lcha_structure):
        assert "12" in lcha_structure["parts"]

    def test_part12_has_sections(self, lcha_structure):
        sections = lcha_structure["parts"]["12"]["sections"]
        assert len(sections) > 0

    def test_part12_sections_have_required_fields(self, lcha_structure):
        sections = lcha_structure["parts"]["12"]["sections"]
        required = ["id", "parent_id", "type", "text_for_embedding", "references"]
        for section_id, section_data in sections.items():
            for field in required:
...
class TestPart13:
    """Integration tests for Part 13"""

    def test_part13_exists(self, lcha_structure):
        assert "13" in lcha_structure["parts"]

    def test_part13_has_sections(self, lcha_structure):
        sections = lcha_structure["parts"]["13"]["sections"]
        assert len(sections) > 0

    def test_part13_sections_have_required_fields(self, lcha_structure):
        sections = lcha_structure["parts"]["13"]["sections"]
        required = ["id", "parent_id", "type", "text_for_embedding", "references"]
        for section_id, section_data in sections.items():
            for field in required:
...
class TestPart14:
    """Integration tests for Part 14: SCHEDULES AND ANNEXES"""

    def test_part14_exists(self, lcha_structure):
        assert "14" in lcha_structure["parts"]

    def test_part14_has_sections(self, lcha_structure):
        sections = lcha_structure["parts"]["14"]["sections"]
        assert len(sections) > 0

    def test_part14_sections_have_required_fields(self, lcha_structure):
        sections = lcha_structure["parts"]["14"]["sections"]
        required = ["id", "parent_id", "type", "text_for_embedding", "references"]
        for section_id, section_data in sections.items():
            for field in required:
...
class TestAllParts:
    ...
    def test_parsed_parts_count(self, lcha_structure):
        """Should have all 14 parts parsed"""
        assert len(lcha_structure["parts"]) >= 14
